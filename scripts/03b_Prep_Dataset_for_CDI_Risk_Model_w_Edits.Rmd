...
med <- med %>% 
  ...
  mutate(PPI = if_else(pharm_class=="PROTON-PUMP INHIBITORS", 1, 0),    
         ...
         anyGAS = if_else(pharm_class %in% c("PROTON-PUMP INHIBITORS", "ANTACIDS", "HISTAMINE H2-RECEPTOR INHIBITORS"), 1, 0),
         LAX = if_else(pharm_class %in% c("LAXATIVES AND CATHARTICS", "LAXATIVES, LOCAL/RECTAL"), 1, 0),
         ABX = if_else(thera_class %in% c("ANTIBIOTICS", "ANTIFUNGALS", "ANTIINFECTIVES/MISCELLANEOUS", "ANTIPARASITICS",
                                          "ANTIVIRALS"), 1, 0)) %>% 
