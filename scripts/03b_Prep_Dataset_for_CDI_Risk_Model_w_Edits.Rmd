...
med <- med %>% 
  ...
  mutate(PPI = if_else(pharm_class=="PROTON-PUMP INHIBITORS" | str_detect(name, fixed("PANTOPRAZOLE", ignore_case=TRUE)), 1, 0),    
         GAS = if_else(pharm_class %in% c("ANTACIDS", "HISTAMINE H2-RECEPTOR INHIBITORS"), 1, 0),
         anyGAS = if_else(pharm_class %in% c("PROTON-PUMP INHIBITORS", "ANTACIDS", "HISTAMINE H2-RECEPTOR INHIBITORS") | str_detect(name, fixed("PANTOPRAZOLE", ignore_case=TRUE)), 1, 0),
         LAX = if_else(pharm_class %in% c("LAXATIVES AND CATHARTICS", "LAXATIVES, LOCAL/RECTAL"), 1, 0),
         ABX = if_else(thera_class %in% c("ANTIBIOTICS", "ANTIFUNGALS", "ANTIINFECTIVES/MISCELLANEOUS", "ANTIPARASITICS","ANTIVIRALS") | str_detect(name, fixed("VANCOMYCIN")) | str_detect(name, fixed("CEFTRIAXONE")) | str_detect(name, fixed("CEFEPIME")) | str_detect(name, fixed("LEVOFLOXACIN")) | str_detect(name, fixed("CIPROFLOXACIN")), 1, 0))
...
# p-value<0.05, thus, HACIF and anyGAS are DEPENDENT
...
# p-value<0.05, thus, HACIF and LAX are DEPENDENT
