...
# p-value < .05, thus, HACIF and AGE are NOT independent, aka they are related
...
# p-value < .05, thus, HACIF and ABX are DEPENDENT

fisher.test(df_CDI$HACDIF, df_CDI$ABX)
# OR=2.16, meaning patients who received ABX up to one day before HAI diagnosis are 2.16x more likely to be diagnosed with HA-CDIF than those who do not receive ABX
```
...
# p>0.05, thus, we fail to reject the null hyp -- HACIF and GAS are INDEPENDENT 

fisher.test(df_CDI$HACDIF, df_CDI$GAS)
# OR=1.13 but 95% CI includes 1 and p>.05, so cannot claim any meaningful extra odds with obtaining gastric acid supplement or laxatives in relation to HACDIF
```
...
# p<.05, thus, HACIF and PPI are DEPENDENT

fisher.test(df_CDI$HACDIF, df_CDI$PPI)
# OR=0.58, meaning patients who received a gastric acid supplement or laxative (GAS) up to one day before HAI diagnosis are 0.58x more likely to be diagnosed with HA-CDIF than those who do not receive GAS
```
