...
*Outcome is binary.*
  ...
  * `cdif_gt_2_days` --> whether patient is diagnosed with C difficile > 2 days after hospital admission (binary)
...
*All predictors are binary.*
  ...
  * THINK ABOUT INCLUDING THESE LATER!!!
...
# As per matrix, the data meets the assumption of absence of multicollinearity. The IVs are not highly correlated with each other. 
```

## Linearity of Independent Variables and Log Odds
Continuous IVs must be linearly related to the logit of the outcome. We can test this by visualizing scatterplot between each predictor and logit values. Since all IVs are binary, can ignore.

## Large Sample Size
General guideline is to have at least 10 cases with least frequent outcome for each IV/predictor variable in your model.

Since our model will include 5 predictors and the probability of Cdiff is 101/54955 (.0018), then we would like a sample size of about 28000 (10*5/.0018).

# Build logistic regression model 
```{r Build logistic regression model}
model <- glm(data = df, formula = cdif_gt_2_days ~ readmit + age_gte_65 + PPI + GAS + ABX, family = binomial)

summary(model)
```

Since our model suggests `ABX` to be the least significant predictor, we will re-run this model WITHOUT it.

```{r Build log reg model 2}
model2 <- glm(data = df, formula = cdif_gt_2_days ~ readmit + age_gte_65 + PPI + GAS, family = binomial)
...
```{r Calculate AUROC}
logit_prob <- predict(model2, newdata = df, type = 'response')

logit_roc <- pROC::roc(df$cdif_gt_2_days ~ logit_prob, plot = T, print.auc = T)
```

The AUROC is 0.74 which shows that the model is moderately accurate at predicting whether a patient will be diagnosed with C diff given the following risk factors (age>=65, readmit, received PPI, received gastric acid supp or laxative, received abx). 

```{r Previous code attempts, disregard for now, echo=F, eval=F}
#prob_thresh <- 0.8
#logit_data <- df %>% add_column(logit_prob)
#logit_plot <- plot(logit_data)
```
