...
m <- glm(data = df, formula = HACDIF ~ readmit + age_gte_65 + PPI + GAS + ABX, family = binomial)
...
Since our model suggests `GAS` to be the least significant predictor, we will re-run this model WITHOUT it.

Since our model suggests `GAS` to be the least significant predictor, we will re-run this model WITHOUT it.
```{r Build log reg model 2}
m2 <- glm(data = df, formula = HACDIF ~ age_gte_65 + readmit + PPI + ABX, family = binomial)
