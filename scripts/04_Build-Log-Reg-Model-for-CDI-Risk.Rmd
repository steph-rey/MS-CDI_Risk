...
Since our model will include 5 predictors and the probability of Cdiff is 101/55037 (.0018), then we would like a sample size of about 28000 (10*5/.0018). If we include ED_dispo and DBM, we would have 7 predictors, and thus, would aim for a sample size of about 39000.
...
Since our model suggests `PPI` to be the least significant predictor, we will re-run this model WITHOUT it.

Since our model suggests `PPI` to be the least significant predictor, we will re-run this model WITHOUT it.
```{r Build log reg model 2}
m2 <- glm(data = df, formula = HACDIF ~ age_gte_65 + readmit + GAS + ABX, family = binomial)
...
The AUROC is 0.97 which shows that the model is very accurate at predicting whether a patient will be diagnosed with C diff given the following risk factors (age>=65, readmit, received PPI, received gastric acid supp or laxative, received abx). 
